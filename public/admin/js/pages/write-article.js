!function (t) {
    ...
    const app = Vue.createApp({
        ...
        mounted(){
            ...
            // 获取URL id
            this.id = utils.get.query.string('id')
        ...
        methods:{
                    ...
                    upload: {
                        ...
                        handler: (files) => {
                            ...
                            axios.post('/admin/handle/upload', params, {
                                ...
                                } else this.contentEditor.tip(res.data.msg, 2000)
                            })
                        },
                        filename: (name) => {
            ...
            initData(id = this.id, is_load = false){
                ...
                POST('/admin/WriteArticle', { id }).then((res) => {
                    if(res.code == 200){
                        ...
                        if (!utils.is.empty(id)) {
                            ...
                            // 标签和分类数据转数组
                            if (utils.is.empty(this.article.article.tag_id)) this.tags   = ''
                            else {
                                this.tags  = this.article.article.tag_id.split('|');
                                // 过滤空字段
                                this.tags  = this.tags.filter((s)=>{ return s && s.trim() })
                            }
                            if (utils.is.empty(this.article.article.sort_id)) this.sorts = ''
                            else {
                            ...
                            // select2 文章权限 初始化
                            if (!utils.is.empty(opt)) {
                                if (!utils.is.empty(opt.auth)) {
                                    auth.forEach(item=>{
                                        if (item.id == opt.auth) {
                                            item.selected = true
                                ...
                                if (!utils.is.empty(opt.comments)) {
                        ...
                        this.article.sort.forEach((item)=>{
                            ...
                            delete item.name
                            // 设置预选中
                            if(!utils.is.empty(id) && utils.in.array(item.id,this.sorts)) item.selected = true
                        });
                        // 处理标签数据
                        this.article.tag.forEach((item)=>{
                            ...
                            delete item.name
                            // 设置预选中
                            if(!utils.is.empty(id) && utils.in.array(item.id,this.tags)) item.selected = true
                        });
                        // select2 分类 初始化
                        ...
                        if (is_load) Tool.Notyf('数据已刷新！', 'success')
                    }
                })
            },
            ...
            btnSave(id = this.id, jump = false){
                ...
                // 提交数据
                POST('/admin/method/SaveArticle', params).then((res) => {
                    if (res.code == 200) {

                        this.id = res.data
                        if (utils.is.empty(utils.get.query.string('id'))) if (history.pushState) history.pushState({id:this.id}, '撰写文章', window.location.href + '?id=' + this.id)
            ...
            insertTag(opt = null) {
                ...
                if (opt == 'insert_color_text') {
                    ...
                    let url     = (utils.is.empty(this.insert_btn.url)) ? '' : ` url='${this.insert_btn.url}'`
            ...
            initInsertData(){
                ...
                GET('/admin/json/emoji.json').then(() => {
            ...
            insertFile(url = this.insert_files.url){
                ...
                if (!utils.is.empty(url)) {
                    ...
                    } else this.contentEditor.insertValue(`${url}`)
                }
            },
            ...
            uoloadFiles(event){
                ...
                for (let item of files) {
                    ...
                    POST('/admin/handle/upload', params, {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
                    }).then(res => {
                        ...
                        } else this.contentEditor.tip(res.msg, 2000)
                    })
                }
                event.target.value = ''
            }
