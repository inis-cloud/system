!function (t) {
    ...
    const app = Vue.createApp({
        ...
        methods: {
            ...
            initData(){
                this.login_account = inisTemp.data('login_account')
                // 支付宝收款码
            ...
            btnSave(){
                ...
                if(inisHelper.is.empty(this.login_account.account)){
                    $.NotificationApp.send("帐号错误！", "帐号不得为空!", "top-right", "rgba(0,0,0,0.2)", "warning");
                }else if(inisHelper.is.empty(this.login_account.email)){
                    $.NotificationApp.send("邮箱错误！", "邮箱不得为空!", "top-right", "rgba(0,0,0,0.2)", "warning");
                    ...
                    axios.post('/index/method/EditProfile', params).then((res) => {
                    ...
                    $.NotificationApp.send("邮箱错误！", "邮箱格式不正确!", "top-right", "rgba(0,0,0,0.2)", "warning");
                }
            },
            ...
            upload(e){
                ...
                if(file.size > 5*1024*1024) $.NotificationApp.send("验证错误！", "上传文件不得大于5MB！", "top-right", "rgba(0,0,0,0.2)", "warning");
                    ...
                    axios.post("/index/handle/uploadHead", params, {
                        headers: {
