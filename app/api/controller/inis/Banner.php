...
use app\validate\{Banner as vBanner};
use think\exception\ValidateException;
use app\model\mysql\{Banner as BannerModel};
...
class Banner extends Base
{
    ...
    public function one($param)
    {
        ...
        // 设置缓存名称
        $cache_name = 'banner?id='.$param['id'];
        
        // 检查是否存在请求的缓存数据
        if (Cache::has($cache_name) and $api_cache and $cache) $data = json_decode(Cache::get($cache_name));
        else {
            // 获取数据库数据
            $data = BannerModel::ExpandAll((array)$param['id']);
            Cache::tag(['banner',$cache_name])->set($cache_name, json_encode($data));
        }
    ...
    public function all($param)
    {
        ...
        // 设置缓存名称
        $cache_name = 'banner?page='.$param['page'].'&limit='.$param['limit'].'&order='.$param['order'];
        ...
        if (Cache::has($cache_name) and $api_cache and $cache) $data = json_decode(Cache::get($cache_name));
            ...
            // 获取数据库数据
            $data = BannerModel::ExpandAll(null, $opt);
            Cache::tag(['banner'])->set($cache_name, json_encode($data));
        }
    ...
    public function sql($param)
    {
        ...
        // 设置缓存名称
        $cache_name = 'banner/sql?page='.$page.'&limit='.$limit.'&order='.$order.'&where='.$where.'&whereOr='.$whereOr;
    ...
    public function saves($param)
    {
        ...
        // 允许用户提交并存储的字段
        $obtain = ['title','description','url','img','opt','longtext'];
        $item   = isset($param['id']) ? BannerModel::findOrEmpty((int)$param['id']) : new BannerModel;

        try {
                
            validate(vBanner::class)->check($param);
