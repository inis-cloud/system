...
class Banner extends Base
{
    ...
    public function one($param)
    {
        ...
        // 设置缓存名称
        $cache_name = json_encode(array_merge(['IAPI'=>'banner'], $param));
        ...
        if (Cache::has($cache_name) and $this->ApiCache) $data = json_decode(Cache::get($cache_name));
            ...
            if ($this->ApiCache) Cache::tag(['banner',$cache_name])->set($cache_name, json_encode($data));
        }
    ...
    public function all($param)
    {
        ...
        if (Cache::has($cache_name) and $this->ApiCache) $data = json_decode(Cache::get($cache_name));
            ...
            if ($this->ApiCache) Cache::tag(['banner'])->set($cache_name, json_encode($data));
        }
    ...
    public function sql($param)
    {
        ...
        // 设置缓存名称
        $cache_name = json_encode(array_merge(['IAPI'=>'banner/sql','where'=>$where,'whereOr'=>$whereOr], $param));
    ...
    public function save($param)
    {
        ...
        if (empty($param['id'])) $item = new BannerModel;
        else {
            $item = BannerModel::findOrEmpty((int)$param['id']);
