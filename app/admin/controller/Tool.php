...
class Tool
{
    ...
    // 邮箱发送
    public function sendMail(array $opt = [])
    {
        // 第二参数默认配置
        $data = ['email'=>'','title'=>'消息提醒','content'=>''];
        
        // 重组第二参数
        foreach ($data as $key => $val) if (!in_array($key, $opt)) $data[$key] = $val;
        foreach ($opt  as $key => $val) $data[$key] = $val;
        
        // 邮箱配置
        $config   = Options::field(['opt'])->where(['keys'=>'config:email-serve'])->find()['opt'];
        // 获取站点名称
        $site     = Options::field(['opt'])->where(['keys'=>'site'])->find()['opt'];
        ...
        $mail->CharSet  = $config->encoded;
        
        // 设置发件人姓名（昵称） 任意内容，显示在收件人邮件的发件人邮箱地址前的发件人姓名
        $mail->FromName = $site->title;
        ...
        if (is_string($data['email'])) $mail->addAddress(empty($data['email']) ? $config->email : $data['email'], '');
        else if (is_array($data['email'])) foreach ($data['email'] as $val) $mail->addAddress(empty($val) ? $config->email : $val, '');
