...
class Users extends Model
{
    ...
    {
        // 第二参数默认配置
        $config = ['page'=>1,'limit'=>5,'order'=>'create_time asc','withoutField'=>['password','opt'],'whereOr'=>[],'where'=>[]];
        ...
        $users = self::whereOr($config['whereOr'])->withAttr('expand', function ($value,$data) {
        ...
        // 只有单条数据
        if (!empty($id) and is_numeric($id)) $result = $users[0];
        else if (is_array($id)) $result = $users;
        else $result = $data;
